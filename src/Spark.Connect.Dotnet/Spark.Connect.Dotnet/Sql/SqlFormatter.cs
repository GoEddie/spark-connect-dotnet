...
{
    /// <summary>
    /// Formats the SQL including the new token parsing options in SparkSession.Sql - note you can pass a DataFrame and this will create a temp name, call df.CreateOrReplaceTempView with the
    ///     unique name and use the name in the query
    /// </summary>
    /// <param name="source">Ensure if the input is from an external source that you validate it, we pass the SQL on with the tokens removed, there is no validation done here</param>
    /// <param name="args">Encure if args are from an external source that you validat them, we replace tokens in the SQL, there is no validaton done here</param>
    /// <returns></returns>
    /// Example:
    ...
    public static string Format(string source, IDictionary<string, object> args)
    {
        ...
        foreach (var item in args)
        {
            ...
            if (item.Value is Column col)
            {
                ...
                {
                    outputSql = outputSql.Replace($"{{{item.Key}}}", col.Expression.UnresolvedAttribute.UnparsedIdentifier);
                }
