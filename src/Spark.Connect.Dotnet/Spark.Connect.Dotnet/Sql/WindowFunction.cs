...
public class WindowFunction : Functions
{
    ...
    /// ```
    ///
    /// Note the WindowFunction.Window to get to the function and then the special window.start column, the schema looks like:
    ///
    /// ```
    ///  root
    ...
    public static Column Window(Column timeColumn, string windowDuration, string? slideDuration = null, string? startTime = null)
    {
        if (!string.IsNullOrEmpty(slideDuration) && !string.IsNullOrEmpty(startTime))
        {
            return new Column(FunctionWrappedCall("window", false, timeColumn, Lit(windowDuration), Lit(slideDuration), Lit(startTime)));
        }
    ...
    public static Column Window(string timeColumn, string windowDuration, string? slideDuration = null, string? startTime = null) => Window(Col(timeColumn), windowDuration, slideDuration, startTime);
