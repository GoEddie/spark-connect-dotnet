...
public class PipelineSinkProcessor
{
    private readonly SparkSession _spark;

    public PipelineSinkProcessor(
        SparkSession spark,
        PipelineGraph graph,
        string sinkName,
        DataFrame source,
        ...
        string? sourceCodeDefinitionPath = null)
    {
        ...
        if (options != null)
        {
            sinkDetails.Options.Add(options);
        }

        if (!string.IsNullOrEmpty(format))
        {
            sinkDetails.Format = format;
        }
