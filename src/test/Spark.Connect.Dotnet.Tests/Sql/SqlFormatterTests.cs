...
public class SqlFormatterTests : E2ETestBase
{
    ...
    {
        var df = Spark.Range(100);
        var output = SqlFormatter.Format("SELECT {idCol} FROM {df1}",
            new Dictionary<string, object> { { "df1", df }, { "idCol", df["id"] } });
        Spark.Sql(output).Show();
    }
    ...
    {
        var df = Spark.Range(100);
        var output = SqlFormatter.Format("SELECT * FROM {df1} WHERE id = {three}",
            new Dictionary<string, object> { { "df1", df }, { "three", 3 } });
        Spark.Sql(output).Show();
    }
    ...
    public void SelectWhereColEqualsString_Test()
    {
        ...
        var output = SqlFormatter.Format("SELECT * FROM {df1} WHERE str = {hello}",
            new Dictionary<string, object> { { "df1", df }, { "hello", "hello" } });
        Spark.Sql(output).Show();
    }
}